			    	 ...
			    	 GtkAllocation *allocation)
{
    ...
    if (width <= allocation_width) {
        ...
        gint slider_space;
        reached_end = FALSE;
        slider_space = 2 * (path_bar->priv->spacing + path_bar->priv->slider_width);
    ...
    for (list = first_button; list; list = list->prev) {
        ...
        if (need_sliders && direction == GTK_TEXT_DIR_RTL) {
            ...
            if (need_sliders && direction == GTK_TEXT_DIR_LTR) {
                if (child_allocation.x + child_allocation.width + path_bar->priv->spacing + path_bar->priv->slider_width - (offset) > widget_allocation.x + allocation_width) {
                    break;
                }
            }
        }
