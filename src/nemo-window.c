...
{
    NemoWindowSlot *current_slot = nemo_window_get_active_slot (window);
    GFile *location = nemo_window_slot_get_location (current_slot);

    if (location != NULL) {
        NemoWindowOpenFlags flags = 0;

        int new_slot_position = g_settings_get_enum (nemo_preferences, NEMO_PREFERENCES_NEW_TAB_POSITION);
        ...
        NemoWindowSlot *new_slot = nemo_window_pane_open_slot (current_slot->pane, flags);
                 ...
                 NemoWindowSlot *slot)
{
    ...
    if (!slot_is_active ||
        ...
        NemoNotebook *notebook = NEMO_NOTEBOOK (slot->pane->notebook);
...
nemo_window_save_geometry (NemoWindow *window)
{
    ...
    if (gtk_widget_get_window (GTK_WIDGET (window)) && !nemo_window_is_desktop (window)) {
        char *geometry_string = eel_gtk_window_get_geometry_string (GTK_WINDOW (window));
                ...
                NemoWindowSlot *slot)
{
    ...
    if (slot == nemo_window_get_active_slot (window)) {
        ...
        if (!g_settings_get_boolean (nemo_preferences, NEMO_PREFERENCES_ALWAYS_USE_BROWSER)) {
            char *full_title = g_strdup_printf (_("%s - File Browser"), slot->title);
                   ...
                   gboolean setting)
{
    ...
    for (l = window->details->panes; l != NULL; l = l->next) {
        ...
        GtkAction *action = gtk_action_group_get_action (pane->action_group,
                              NEMO_ACTION_SEARCH);
