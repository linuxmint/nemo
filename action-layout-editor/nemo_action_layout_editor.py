...
class NemoActionsOrganizer(Gtk.Box):
    def __init__(self, window, builder=None):
        ...
        self.blank_icon_menu_item = Gtk.ImageMenuItem(label=_("No icon"), image=Gtk.Image(icon_name="xapp-checkbox-symbolic"))
        ...
        item = Gtk.ImageMenuItem(label=_("New submenu"), image=Gtk.Image(icon_name="xapp-pan-end-symbolic"))
        item.connect("activate", self.on_new_submenu_clicked)
        menu.add(item)

        item = Gtk.ImageMenuItem(label=_("New separator"), image=Gtk.Image(icon_name="xapp-list-remove-symbolic"))
    ...
    def update_row_controls(self):
        ...
        if row is not None:
            ...
            if row_type == ROW_TYPE_ACTION and row.get_custom_label() is not None:
                self.name_entry.set_icon_from_icon_name(Gtk.EntryIconPosition.SECONDARY, "xapp-edit-delete-symbolic")
...
class EditorWindow():
    def __init__(self):
        ...
        item = Gtk.ImageMenuItem(label=_("Open user actions folder"), image=Gtk.Image(icon_name="xapp-folder-symbolic", icon_size=Gtk.IconSize.MENU))
