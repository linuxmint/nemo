                     ...
                     gboolean                 from_event)
{
    ...
    gtk_label_set_text (GTK_LABEL (item->toggle_label_widget),
                        complex_mode ? _("Show less actions") : _("Show more actions"));
...
nemo_context_menu_menu_item_init (NemoContextMenuMenuItem *item)
{
    ...
    update_toggle_state (item, FALSE);
                                   ...
                                   GdkEventCrossing *event)
{
    ...
    update_toggle_appearance_from_event (widget, event->x, event->y);
